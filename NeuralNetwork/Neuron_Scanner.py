...
if __name__ == '__main__':
    ...
    res = NN.Neuron_binary_scanner(n_iter, max_epoch, k_range, lr_range, patterns, labels, label0, label1, from_scratch=from_scratch)
    
    if not os.path.exists(script_directory+f'/Neuron Not PCA'):
        os.makedirs(script_directory+f'/Neuron Not PCA')
    
    if not os.path.exists(script_directory+f'/Neuron Not PCA/score_heatmaps'):
        os.makedirs(script_directory+f'/Neuron Not PCA/score_heatmaps')
    ...
    fig.suptitle(f' Epoch {max_epoch}')
    fig.colorbar(spec_colormesh, orientation='vertical')
    fig.savefig(script_directory+f'/Neuron Not PCA/score_heatmaps/heatmap_{max_epoch}_epoch.png', dpi=240)
    plt.close(fig)
    ...
    for i,k in enumerate(k_range):
        ...
        fig_loss.savefig(script_directory+f'/Neuron Not PCA/loss_heatmaps/heatmap_{k}_fold.png', dpi=240)
        plt.close(fig_loss)
...
fig2.suptitle(f'Metrics: {k_range[0]}-folds, {lr_range[0]:0.3e} LR')
...
def update_metrics_2(val):
    ...
    ax2[1].set_xlabel('Epochs')
    fig2.suptitle(f'Metrics: {k_range[i_k]}-folds, {lr_range[i_lr]:0.3e} LR')
...
axSlide_lr3 = fig3.add_axes([0.04, 0.55, 0.05, 0.4])
...
axSlide_ep3 = fig3.add_axes([0.04, 0.05, 0.05, 0.4])
